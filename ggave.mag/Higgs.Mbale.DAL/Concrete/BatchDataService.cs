  ...
  public  class BatchDataService : DataServiceBase, IBatchDataService
    {
       ...
       public BatchDataService(IUnitOfWork<MbaleEntities> unitOfWork)
        ...
        public IEnumerable<Batch> GetTenBatchesForAParticularBranch(long branchId)
        {
            return this.UnitOfWork.Get<Batch>().AsQueryable().Where(e => e.Deleted == false && e.BranchId == branchId).OrderByDescending(e =>e.CreatedOn).Take(25);
        }
        public IEnumerable<Batch> GetAllBatchesForBrandDelivery(long branchId)
        {
            return this.UnitOfWork.Get<Batch>().AsQueryable().Where(e => e.Deleted == false && e.BranchId == branchId && e.BrandBalance > 0).OrderByDescending(e => e.CreatedOn).Take(15);
        }
        ...
        public IEnumerable<DeliveryBatch> GetBatchDeliveries(long batchId)
        {
            return this.UnitOfWork.Get<DeliveryBatch>().AsQueryable().Where(e => e.BatchId == batchId);
    
        }

        public IEnumerable<FlourTransferBatch> GetBatchFlourTransfers(long batchId)
        {
            return this.UnitOfWork.Get<FlourTransferBatch>().AsQueryable().Where(e => e.BatchId == batchId);

        }

        public IEnumerable<CashSaleBatch> GetBatchCashSales(long batchId)
        {
            return this.UnitOfWork.Get<CashSaleBatch>().AsQueryable().Where(e => e.BatchId == batchId);

        }

        public IEnumerable<DeliveryBatch> GetAllBatchesForADelivery(long batchId)
        {
