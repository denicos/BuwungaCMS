...
angular
    ...
    function ($scope, $http, $filter, $location, $log, $timeout, $modal, $state, uiGridConstants, $interval, usSpinnerService) {
        ...
        //$http.get('/webapi/MaizeBrandStoreApi/GetAllMaizeBrandStoreForAParticularBranchToDeliver?branchId=' + branchId
        //           ).then(function (payload) {
        //               $scope.retrievedBatches = payload.data;
        //               $scope.batches = $scope.retrievedBatches;
        //           });
        ...
        $scope.Save = function (cashSale) {
            ...
            if (cashSale.ProductId == 1) {
                if (cashSale.Batches != null && cashSale.Batches != "undefined") {
                    ...
                    if (cashSale.selectedGrades) {
                        ...
                        $timeout(function () {
                            $scope.showMessageCheckGrade = false;
                            $state.go('cashsale-list', { 'branchId': $scope.branchId });
                ...
                if (cashSale.Batches != null && cashSale.Batches !== 'undefined') {
                    ...
                    if ($scope.batchBrandQuantity == cashSale.Quantity) {
                        ...
                        $scope.showMessageBatchQuantitiesDoNotMatch = true;

                        $timeout(function () {
                            $scope.showMessageBatchQuantitiesDoNotMatch = false;
            ...
            //    if (cashSale.Batches != null && cashSale.Batches !== 'undefined')
            //    {
            //        $scope.batchQuantity = 0;
            //        angular.forEach(cashSale.Batches, function (value, key) {
            //             $scope.batchQuantity = value.Quantity + $scope.batchQuantity;
                       
            //        });
                    
            //        if ($scope.batchQuantity >= cashSale.Quantity)
            //        {
            //            $scope.TotalAmount = cashSale.Price * cashSale.Quantity;
            //            $scope.TotalQuantity = cashSale.Quantity;
            //        }
            //        else
            //        {
            //            $scope.showMessageBatchQuantityNotEnough = true;
    ...
    .module('homer').controller('BranchCashSaleController', ['$scope', 'ngTableParams', '$http', '$filter', '$location', 'Utils', 'uiGridConstants',
        function ($scope, ngTableParams, $http, $filter, $location, Utils, uiGridConstants) {
            ...
            $scope.LoadMore = function () {
                ...
                var promise = $http.get('/webapi/CashSaleApi/GetAllCashSalesForAparticularBranch?offSet=' + $scope.offset + '&pageSize=' + $scope.loadPageSize);
                promise.then(
                    function (payload) {
            ...
            var promise = $http.get('/webapi/CashSaleApi/GetAllCashSalesForAparticularBranch?offSet=' + $scope.offset + '&pageSize=' + $scope.pageSize);
            promise.then(
                function (payload) {
